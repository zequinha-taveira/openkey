...
2. **Respeito aos ADRs**: Antes de sugerir refatorações estruturais em pacotes como `firmware` ou `host/sdk`, consulte os [ADRs (Architecture Decision Records)](docs/adr/README.md).
3. **Manutenção da Documentação**: Qualquer alteração em APIs públicas ou protocolos deve vir acompanhada da atualização correspondente em `docs/api/` e `docs/protocols/`.
